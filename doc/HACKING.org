...
- =nonascii.sh=: Test displaying non-ascii (double-width and other
  ...
  (=M-x view-hello-file=)
- =parser.sh=: Exercise VT parser behaviour in the face of error
  conditions (illegal control sequences) and similar edge cases.
- =scrollback.sh=: Scrollback (page history) support
- =tabs.sh=: Setting and clearing tabs; jumping to prev/next tab
  position.
